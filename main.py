...
def create_task(body: TaskIn, user: sqlite3.Row = Depends(get_current_user)):
    ...
    with get_db() as db:
        db.execute(
            "INSERT INTO tasks (id, user_id, title, description, status, priority, repeat, category_id, due_date, created_at, updated_at) "
            "VALUES (?,?,?,?,?,?,?,?,?,?,?)",
            (tid, user["id"], body.title, body.description, body.status, body.priority, body.repeat,
             body.category_id, body.due_date, now, now),
        )
...
# ---------------------------------------------------------------------------
def category_public(c: sqlite3.Row) -> dict:
    return {"id": c["id"], "name": c["name"], "color": c["color"]}


@api.get("/categories")
def list_categories(user: sqlite3.Row = Depends(get_current_user)):
    with get_db() as db:
        rows = db.execute("SELECT * FROM categories ORDER BY created_at ASC").fetchall()
    return [category_public(c) for c in rows]


@api.post("/categories", status_code=201)
def create_category(body: CategoryIn, user: sqlite3.Row = Depends(get_current_user)):
    cid = str(uuid.uuid4())
    with get_db() as db:
        db.execute(
            "INSERT INTO categories (id, name, color, created_at) VALUES (?,?,?,?)",
            (cid, body.name, body.color, dt.datetime.utcnow().isoformat()),
        )
...
def delete_category(category_id: str, user: sqlite3.Row = Depends(get_current_user)):
    with get_db() as db:
